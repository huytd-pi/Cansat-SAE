...
def readSensorData():
	...
	zero = "MQ135 RZero: %s" % round(rzero)
	correct="\nCorrected RZero: %s" % round(correctedRZero)
	res="\nResistance: %s" % round(resistance)
	ppmv="\nPPM: %s" % round(ppm)
	ppmcorrect="\nCorrected PPM: %s ppm" % round(correctedPPM)
	return "\r\n-----MQ135 DATA-----\r\n"+zero+correct+res+ppmv+ppmcorrect